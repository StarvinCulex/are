...
impl Cosmos {
    ...
    >(
        ...
        macro_rules! bench {
            () => {
                &mut self.angelos.stats.lock().unwrap().benchmark
            };
        }

        crate::benchmark_time!(
            ["message tick step#prepare", bench!()]
            let thread_count = self.angelos.conf.runtime.thread_count;
        ...
        crate::benchmark_time!(
            ["message tick step#ground_hear", bench!()]
            jobs::work(
                workers.iter_mut(),
        ...
        crate::benchmark_time!(
            ["message tick step#pos2weakmob", bench!()]
            self.pos_to_weak_mob(mob_pos_messages, &mut mob_messages);
        );

        crate::benchmark_time!(
            ["message tick step#mobhear", bench!()]
            ReadGuard::with(&self.angelos.pkey, |guard| {
                jobs::work(
                    workers.iter_mut(),
    ...
    >(
        ...
        crate::benchmark_time!(
            ["order tick step#groundorder", bench!()]
            jobs::work(
                workers.iter_mut(),
